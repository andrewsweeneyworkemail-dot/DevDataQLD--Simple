...
def set_date_range(page, start: str, end: str) -> bool:
    # simplest CI strategy: look for two inputs and fill them
    try:
        start_inp = page.locator("input").filter(has_text=re.compile("", re.I)).nth(0)
        end_inp = page.locator("input").filter(has_text=re.compile("", re.I)).nth(1)
        start_inp.click()
        start_inp.fill(start)
        end_inp.click()
        end_inp.fill(end)
...
def show_results(page) -> None:
    try_click_many(page, [("text", r"Show Results"), ("role_button", r"Show Results")], timeout=6000)
